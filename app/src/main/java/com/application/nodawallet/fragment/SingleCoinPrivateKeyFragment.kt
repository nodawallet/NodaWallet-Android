...
import kotlinx.android.synthetic.main.fragment_single_coin_private_key.*
 ...
 */
class SingleCoinPrivateKeyFragment : BaseFragment(R.layout.fragment_single_coin_private_key) {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        textSinglePrivatePaste.setOnClickListener {
            val text = clipBoardManager.primaryClip?.getItemAt(0)?.text.toString()
            edtPrivateKey.setText(text)
        }

        btnPrivateKeyImport.setOnClickListener {

             privatekey = edtPrivateKey.text.toString().trim()

            if (privatekey == "") {
                Toast.makeText(activity, getString(R.string.empty_key), Toast.LENGTH_SHORT).show()
            } else if (!WalletUtils.isValidPrivateKey(privatekey)) {
                Toast.makeText(activity, getString(R.string.invalid_key), Toast.LENGTH_SHORT).show()
            } else {

                val wallet = MultiWalletList(getString(R.string.wallet)+" ${size.plus(1)}", R.drawable.ic_eth, 1, privatekey,"privatekey")
    ...
    private fun insertCoin(id:Int?) {
        ...
        var credential = Credentials.create(privatekey)
        var publickey = credential.ecKeyPair.publicKey
        var address = credential.address



        coinlist.add(CoinList("Ethereum",R.drawable.ic_eth,"https://eth.png","ETH","","private",address,privatekey,publickey.toString(),1,"0.000","","","18","ethereum",0.0,0.0,0.0))
